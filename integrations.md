...
## URL Capture (self-hosted capture service)

The capture engine's URL mode renders arbitrary web pages through the
**self-hosted capture service** in this repo (`engines/`) — an open-source,
ScreenshotOne-equivalent screenshot API written **from scratch in Rust**: our
own HTTP server, WebSocket client, Chrome DevTools Protocol client and PNG
pipeline (zero external crates — the only external piece is the Chromium
binary). No proprietary capture service is involved.

The action in `src/convex/captureUrl.ts` reads the service configuration from
the environment:

- `CAPTURE_SERVICE_URL`: where the service listens, e.g. `http://localhost:8080`
- `CAPTURE_SERVICE_API_KEY`: any key from the service's `CAPTURE_API_KEYS`
  (omit only if the service runs in open mode on a trusted network)

Paste them in the project's **Keys / API keys** tab. Without a service URL,
URL captures fail with a clear "missing service" message in the capture job
viewer (the built-in demo page capture works without any service). The action
also re-validates every URL server-side (scheme allowlist, SSRF/private-network
blocks) and walks redirect hops one-by-one before forwarding it.
...
See `docs/self-hosted-capture-service.md` and the in-app `/api` page for the
full API reference and self-hosting guide.
